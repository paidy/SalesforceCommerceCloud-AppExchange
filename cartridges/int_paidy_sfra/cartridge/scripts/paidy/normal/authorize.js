...
var CallService = require('*/cartridge/scripts/paidy/regular/callService');
 ...
 * @returns {Object} Returns normal settlement config data
 */
...
function paidyPay(customer, order) {
    ...
    try {
        ...
        log.error(
            'An error occurred at point # 1 on paydyPay.\nError message:' +
        e.toString() +
        '\norderNo:' +
        order.orderNo +
        '\ncustomerNo:' +
    ...
    try {
        ...
        log.error(
            'An error occurred at point # 2 on paydyPay. Error message:' +
        e.toString() +
        '\ncustomerNo:' +
        order.customerNo +
        '\nnow:' +
    ...
    try {
        ...
        while (it.hasNext()) {
            var pi = it.next();
            if (pi.paymentMethod.toUpperCase().indexOf(Preferences.PaymentType.paydyNomal) !== 0 &&
        ...
        log.error(
            'An error occurred at point # 3 on paydyPay. Error message:' +
        e.toString() +
        '\ncustomerNo:' +
        order.customerNo +
        '\norder.orderNo:' +
    ...
    try {
        ...
        log.error(
            'An error occurred at point # 4 on paydyPay. Error message:' +
        e.toString() +
        '\ncustomerNo:' +
        order.customerNo +
        '\norder.orderNo:' +
...
function fetchOrder(remote, local) {
    ...
    try {
        var currentOrder = OrderMgr.getOrder(orderRef, local.order.order_token);

        // payment method api path - paidy
        var serviceName = Site.getCurrent().getCustomPreferenceValue(
            'paidy_service_name'
        );
        var payMethod = '/payments/' + remote.id;
        var method = CallService.ServiceType.get;
        var response;
...
/**
 * Validation,the time of  a normal payment confirmation
