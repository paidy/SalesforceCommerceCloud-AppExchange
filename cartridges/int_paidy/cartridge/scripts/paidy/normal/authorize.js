...
var paidyPaymentAPI = require('*/cartridge/scripts/paidy/regular/paidyPayment');
...
function paidyPay(customer, order) {
    ...
    try {
        ...
        while (it.hasNext()) {
            var pi = it.next();
            if (
                pi.paymentMethod.toUpperCase().indexOf(Preferences.PaymentType.paydyNomal) !== 0 &&
...
function fetchOrder(remote, local) {
    ...
    // execute Paidy Nomal payment API
