 ...
 * add class attribute "billing-submit" to submit button.
 <script src="${URLUtils.staticURL('paidy_normal.js')}"></script>
 */
(function(global) {
  'use strict';
  ...
  /**
   * Call "onPaidyReady" when the Paidy normal payment is used
   */
  ...
  /**
   * Control  the Paidy normal payment pop-up event
   */
    ...
    return function(result) {
      ...
      switch (result.status.toLowerCase()) {
        case 'authorized':
          ...
          form.method = 'POST';
          form.action = Urls.doPaidyNormalPlaceOrder;
  ...
  function createOrder(cb) {
    ...
    $.ajax({
      method: 'POST',
      dataType: 'json',
      url: Urls.doPaidyNormalCOSummarySubmit,
      data: {
        format: 'ajax',
        csrf_token: csrfToken
      }
  ...
  document.addEventListener('DOMContentLoaded', function(event) {
    ...
    $.getJSON(Urls.getPaidyNormalPaidyConfig, function(data) {
      ...
      if (data.payment_method == 'PAIDY_NORMAL') {
        ...
        Array.prototype.forEach.call(
          ...
          function(form) {
            form.action = Urls.doPaidyNormalCOSummarySubmit;
          }
        );
        Array.prototype.forEach.call(
          document.querySelectorAll(buttonSelector),
